...
import numpy as np
import sys
sys.path.append('/homeappl/home/astuke/anaconda3/lib/python3.5/site-packages')
import sklearn
...
def f(x):
    ...
    data = pd.read_json("data_train_1k.json")
    ...
    if os.path.isfile('results/df_results.json'):
        df_results = pd.read_json('df_results.json', orient='split')
        ...
        df_results.to_json('df_results.json', orient='split')
    else:
        ...
        df_results.to_json('df_results.json', orient='split')
    #row = [str(avg_MAE) + ' ' + str(mbtr_time) + ' ' + str(avg_cv_time) + ' ' + str(alpha) + ' ' + str(gamma) + ' ' + str(sigma2) + ' ' + str(sigma3) + '\n')
    #results.write(str(avg_MAE) + ' ' + str(mbtr_time) + ' ' + str(avg_cv_time) + ' ' + str(alpha) + ' ' + str(gamma) + ' ' + str(sigma2) + ' ' + str(sigma3) + '\n')
