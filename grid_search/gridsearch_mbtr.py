...
filename = 'gs_outfile.txt'
if os.path.exists(filename):
    append_write = 'a' # append if already exists
else:
    append_write = 'w' # make a new file if not
output = open(filename, append_write)

iteration=0
sigma1 = 0.2


#### Load data and divide into different parts for CV
...
for sigma2 in np.logspace(-5, 0, 6):
    for sigma3 in np.logspace(-5, 0, 6):
        ...
        mbtr_train = [mbtr_train_1, mbtr_train_2, mbtr_train_3, mbtr_train_4, mbtr_train_5]        
        mbtr_val = [mbtr_val_1, mbtr_val_2, mbtr_val_3, mbtr_val_4, mbtr_val_5]        
        ...
        for alpha in np.logspace(-10, 0, 11):
            for gamma in np.logspace(-10, 0, 11):
                ...
                print("New combination of hyperparameters")
                print("alpha:", alpha)
                print("gamma:", gamma)
                print("sigma2:", sigma2)
                print("sigma3:", sigma3)
                ...
                if os.path.isfile('results/df_results_mbtr.json'):
                    ...
                    df_results = pd.DataFrame([[iteration, avg_MAE, iteration_time, mbtr_time, avg_cv_time, alpha, gamma, sigma2, sigma3]], columns=['iteration', 'avg_MAE', 'iteration_time', 'mbtr_time', 'avg_cv_time','alpha', 'gamma', 'sigma2', 'sigma3'])
                    df_results.to_json('mbtr/df_results_mbtr.json', orient='split')


time_end = time.time()
total_time=np.round(time_end - time_start, decimals=3)
