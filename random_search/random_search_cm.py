...
for h in range(1, 111):
    ...
    alpha_exp = random.uniform(0, 10.0)
    alpha = 10**-alpha_exp

    gamma_exp = random.uniform(3.0, 10.0)
    gamma = 10**-gamma_exp
    ...
    if os.path.isfile('results/df_results_cm.json'):
        ...
        row = [iteration, avg_MAE, cm_time, avg_cv_time, alpha, gamma, alpha_exp, gamma_exp]
        df_results.loc[len(df_results)] = row
        df_results.to_json('results/df_results_cm.json', orient='split')
        print(df_results)
    else:
        df_results = pd.DataFrame([[1, avg_MAE, cm_time, avg_cv_time, alpha, gamma, alpha_exp, gamma_exp]], columns=['iteration', 'avg_MAE', 'cm_time', 'avg_cv_time','alpha', 'gamma', 'alpha_exp', 'gamma_exp'])
        df_results.to_json('results/df_results_cm.json', orient='split')
